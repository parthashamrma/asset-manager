...
async function importFromExcel(excelFilePath: string) {
  try {
      ...
      try {
        ...
        try {
          const existingStudent = await db.select().from(users)
            .where(users.username.eq(rollNumber));
        ...
        if (subjectsStr) {
          // Handle different subject separators (comma, semicolon, space)
          let subjectCodes: string[] = [];
          if (subjectsStr.includes(';')) {
            subjectCodes = subjectsStr.split(';').map(s => s.trim()).filter(s => s);
...
if (!excelPath) {
  console.log("Usage: npx tsx script/import-excel.ts <path-to-excel-file>");
...
importFromExcel(excelPath).then(() => {
